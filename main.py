...
images_paths = incremental_image_resize(
    "images/starry_night.jpg", "outputs", 100, 4000, 100
)
...
with open("results.csv", "w") as csv_file:
    ...
    for image_path in images_paths:
        ...
        opencv_time = opencv_convolution_applier.benchmark(
            image_path, get_output_path(image_path, "opencv"), iterations=5
        )

        # benchmark the CudaConvolutionApplier
        cuda_time = cuda_convolution_applier.benchmark(
            image_path, get_output_path(image_path, "cuda"), iterations=5
        )
        ...
        simple_time = 0
        if last_simple_time < 1.0:
            simple_time = simple_convolution_applier.benchmark(
                image_path, get_output_path(image_path, "simple"), iterations=5
            )
